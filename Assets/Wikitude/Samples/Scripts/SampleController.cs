...
#region Error Console GUI
	...
	protected static   InstantTrackingState _currentTrackerState = InstantTrackingState.Initializing;
	static             bool                 c_isTracking         = false;
	static             int                  m_Counter            = 0;
	static             int                  m_UnCounter          = 0;
	[NotNull] readonly object               m_BalanceLock        = new object();
	public event Action onSceneLost;
	...
	protected bool IsTracking
	{
		...
		set
		{
			lock (m_BalanceLock)
			{
				var x = new StackTrace(1);
				var trimStart = x.ToString();
				if (c_isTracking == value)
				{
					Debug.Log($"UnChange {m_UnCounter++} to {value}:{trimStart}");
				} else
				{
					Debug.Log($"Change {m_Counter++} to {value}:{trimStart}");
	...
	public virtual void OnSceneLost([CanBeNull] InstantTarget target) {
		if(target == null)  SetSceneActive(false);
	}
	[JetBrains.Annotations.ContractAnnotation("=> halt")]
	protected  virtual void SetSceneActive(bool active)
	{
	}
}
