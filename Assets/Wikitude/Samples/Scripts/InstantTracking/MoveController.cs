...
{
    public static Vector2 ToVector2(this Vector3 x)
    {
        return Math.Abs(x.y) > Mathf.Epsilon? new Vector2(x.x, x.y) :new Vector2(x.x, x.z);
    }
}
...
public class MoveController : MonoBehaviour
{
    ...
    public void SetMoveObject(Transform newMoveObject) {
        if (_controller.ActiveModels.Contains(newMoveObject.gameObject)) {
            ...
            _startTouchPosition = Input.touchSupported&&Input.touchCount>0?Input.GetTouch(0).position:Input.mousePosition.ToVector2();
    ...
    void Update () {
        if (Input.touchCount > 0) {
            ...
            if (_activeObject != null) {
                    Vector2 screenPosForRay  ;
                if(Input.touchSupported)
                {
                    screenPosForRay = (touch.position - _startTouchPosition) + _touchOffset;
                } else
                {

                    screenPosForRay = (Input.mousePosition.ToVector2() - _startTouchPosition) + _touchOffset;
                }
